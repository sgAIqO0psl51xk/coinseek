...
class TwitterAnalyzer:
    def __init__(
        ...
        rate_limit_delay: float = 5.0,  # Increased from 2.0
        max_retries: int = 3
    ):
    ...
    async def with_retry(self, func, *args):
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if "Rate limit exceeded" in str(e) and attempt < self.max_retries - 1:
                    delay = self.rate_limit_delay * (8 ** attempt)  # More aggressive backoff
    ...
    async def create_user_info(self, user) -> UserInfo:
    ...
    async def analyze_user_history(self, user) -> tuple[int, int]:
        try:
            ca_counter = 0
            ticker_counter = 0
    ...
    async def analyze_parent_tweet(self, tweet_id) -> Optional[ParentTweetInfo]:
        try:
            ...
            total_mentions = (parent_user_info.ca_mention_count + 
                            parent_user_info.ticker_mention_count)
    ...
    async def analyze_replies(self, tweet) -> List[TweetReply]:
        ...
        try:
            if tweet.replies:
                for reply in tweet.replies:
                    ...
                    replies.append(TweetReply(user=user_info, content=reply.full_text))
    ...
    async def process_tweet(self, tweet) -> TweetData:
        ...
        if tweet.id in self.processed_tweets:
            ...
            return self.important_tweets_cache.get(tweet.id)

        user_info = await self.create_user_info(tweet.user)
        replies = await self.analyze_replies(tweet)
        parent_tweet = await self.analyze_parent_tweet(tweet.in_reply_to) if tweet.in_reply_to else None
...
async def main():
    ...
    tweet_data_list = await analyzer.analyze_tweets(num_tweets=15)  # Reduced from 20
